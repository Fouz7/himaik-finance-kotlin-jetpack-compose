...
class IncomePagingSource(
...
) : PagingSource<Int, IncomeData>() {
    override fun getRefreshKey(state: PagingState<Int, IncomeData>): Int? {
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, IncomeData> {
        val page = params.key ?: 1
        return try {
            val response = api.incomes(page = page, limit = pageSize)
            if (!response.isSuccessful) throw HttpException(response)
            val body = response.body()
            val items = body?.data ?: emptyList()
            val pagination = body?.pagination
            val totalPages = pagination?.totalPages ?: if (items.isEmpty()) page else page
